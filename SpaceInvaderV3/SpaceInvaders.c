...
#define MAXADC 3910
#define MAX_XVAL 66 //max value for spaceship
...
unsigned long stepSize = MAXADC/MAX_XVAL;
unsigned char xPos =0;//0 to SCREENW-1
unsigned int Ain;
...
//	ADC_Init();
  ...
  Nokia5110_Init();
  unsigned char i;
  unsigned char start=1, end=0;
  const unsigned char col[] ={0,16,32,16};
  ...
  Nokia5110_PrintBMP(xPos, 47, PlayerShip0, 0); // player ship middle bottom
  ...
  while(1){
    ...
    while (start&&!end) {
      ...
      for (i=0;i<4;i++) { // move to the right first then left
				shipPos();
        Nokia5110_ClearInvaders();
        Nokia5110_PrintBMP(col[i], ENEMY10H - 1, SmallEnemy10PointA, 0);
        Nokia5110_PrintBMP(col[i]+16, ENEMY10H - 1, SmallEnemy20PointA, 0);
        Nokia5110_PrintBMP(col[i]+32, ENEMY10H - 1, SmallEnemy30PointA, 0);
        Nokia5110_DisplayBuffer();     // draw buffer
        Delay100ms(5);              // delay 0.5 sec at 16 MHz
        shipPos();
        Nokia5110_ClearInvaders();
        Nokia5110_PrintBMP(col[i], ENEMY10H - 1, SmallEnemy10PointB, 0);
        Nokia5110_PrintBMP(col[i]+16, ENEMY10H - 1, SmallEnemy20PointB, 0);
        Nokia5110_PrintBMP(col[i]+32, ENEMY10H - 1, SmallEnemy30PointB, 0);
        Nokia5110_DisplayBuffer();     // draw buffer
        Delay100ms(5);              // delay 0.5 sec at 16 MHz     
      }
    }
...
//void shipPos(void){
//	//Max X position for ship is 66. Divide ADC input to receive X value.
//	Ain = *ADC_Val();
//	while(Ain>stepSize) { //division
//		xPos+=1;
//		Ain-=stepSize;
//	}
//	Nokia5110_ClearBuffer();
...
void shipPos(void){
  unsigned int ADCValue;
  ADCValue = ADC_In();
	xPos = ADCValue*(SCREENW-18)/4095; // 18: width of the ship
	Nokia5110_ClearBuffer();
  Nokia5110_PrintBMP(xPos, 47, PlayerShip0, 0); // position the ship
//  Nokia5110_PrintBMP(ADC_In()*(SCREENW-18)/4095, 47, PlayerShip0, 0); // position the ship
  Nokia5110_DisplayBuffer();      // draw buffer
};
